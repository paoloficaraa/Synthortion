...
- **Chassis:** fluid `w-full h-full` with 16:10 DAW window scaling (default: 960x600 px, min: 768x480 px, max: 1920x1200 px, aspect ratio locked at 1.6), `1px solid var(--border)` on `.vst-container`, box-drawing corner brackets (`┌ ┐ └ ┘` at 16px `font-ascii` `text-ink-3`, `aria-hidden`), static CRT scanlines (`.vst-container::before` dual `repeating-linear-gradient` + `linear-gradient`, `z-index 40`, `pointer-events none`), and `noise-overlay` turbulence (`opacity 0.025`, `mix-blend-mode overlay`, `z-index 50`).
- **Structure:** 3 columns — left IN rail (48px) · center hub · right OUT rail (48px). Center hub = status bar (50px `h-[50px]` `shrink-0` `border-b border-grid-rule`) + real-time visualizer (~35% height, `basis-[35%]` `flex-grow` `border-b border-grid-rule`) + module grid (~65% height, `basis-[65%]` `flex-grow` `p-3 sm:p-4`, inner faceplate `grid grid-cols-5 h-full border border-grid-rule`).
- **Faceplate:** 5-column grid — DRV | BCR | DLY (×2) | CHR. Each section is a framed module expanding to 100% vertical height: header `h-[24px]` `border-b` with `+ [ CODE ] + [ PWR: ON ] +` and LED power switch, control area with `CalibrationTicks`, bottom bar `h-[16px]` `border-t` with `+ ░▒ CAL.0CODE :: SYS.X ▒░ +`.
- **Meter rails:** `w-[48px] shrink-0 bg-elev-0 flex flex-col items-center py-6 border-r/l border-border` with `CalibrationTicks` mirroring modules (plus `+` at header 50px and visualizer shared boundary hairlines), ladder `w-[8px] h-[256px]` `bg-elev-0` `boxShadow: var(--shadow-well), 0 0 0 1px var(--elev-6)`, top `+ ┌ IN ┐ +` / `0` / `│` gutters, bottom `+ ░▒ └─(1px hairline)─┘ ▒░ +` (CSS `h-px bg-ink-3`, never repeated `─`), bracketed dB readout `[ -INF ]`.
